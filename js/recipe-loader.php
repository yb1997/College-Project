...
// var link = "http://yb1997.github.io/College-Project/JSON/recipes.JSON";
var link = "/College-Project/JSON/recipes.JSON";

function cardResizer (card,height,width,unit="px") {
...
function cardLoader(url) {
  $.getJSON(url, recipes => {
    ...
    for (let i = 0; i < recipes.length; i++,j++) {
      ...
      let card = $("<div class='card' data-url='?id="+ j +"'></div>");// handle click properly
      ...
      let recipeName = $("<a class='recipe-name text-capitalize' href='recipe.php"+ $(card).attr("data-url") +"'></a>");// "<a href='recipe.php" + $(card).attr("data-url") + "' class='recipe-name text-capitalize'></a>"
      let author = $("<p class='author-name'>By </p>");
      let bookmark = $("<i class='fa fa-bookmark fa-2x'></i>");
